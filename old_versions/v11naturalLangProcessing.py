...
def handle_complex_question(question, appliance_data):
    ...
    # Create a matcher to find words related to appliances and energy usage
    matcher = Matcher(nlp.vocab)
    pattern1 = [{"LEMMA": "appliance"}]
    pattern2 = [{"LEMMA": "energy"}, {"LEMMA": "usage"}]
    matcher.add("APPLIANCE", [pattern1])
    matcher.add("ENERGY_USAGE", [pattern2])

    # Find the matches in the question
    matches = matcher(doc)

    # If both patterns are matched, provide a breakdown of energy usage by appliance
    if len(matches) == 2:
