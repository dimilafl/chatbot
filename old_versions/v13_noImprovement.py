...
def handle_complex_question(question, appliance_data):
    ...
    for token in doc:
        ...
        # Check if the token is a proper noun (e.g., an appliance name)
        if token.pos_ == "PROPN" and token.text in appliance_data:
            appliance_name = token.text

    if question_about_appliances and question_about_energy_usage:
        if appliance_name:
            # Respond with the energy usage of the specific appliance
            response = f"The {appliance_name} is using {appliance_data[appliance_name]} kWh of energy."
        else:
